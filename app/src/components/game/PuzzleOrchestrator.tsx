...
export default function PuzzleOrchestrator({ onSolve }: PuzzleOrchestratorProps) {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-8 bg-zinc-900/80 border border-zinc-800 rounded-[40px] backdrop-blur-xl shadow-2xl relative overflow-hidden">
      {/* Background Decor */}
      ...
      <div className="relative z-10 space-y-8 text-center">
        <div className="space-y-2">
          <div className="flex items-center justify-center gap-3 text-primary mb-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em]">Desafio de Inteligência</span>
          </div>
        ...
        {/* Puzzle Visual Area */}
        <div className="py-10 bg-zinc-950/50 rounded-[32px] border border-zinc-800/50 shadow-inner">
           <div className="text-4xl font-mono font-black text-primary tracking-[0.5em] mb-4">
              {puzzle.displayData}
           </div>
           ...
           <AnimatePresence>
             {showHint && (
               <motion.div 
                 ...
                 className="px-6 py-2 bg-primary/10 border border-primary/20 rounded-full inline-flex items-center gap-2"
               >
                 <Lightbulb className="w-3 h-3 text-primary" />
                 <span className="text-[10px] font-bold text-primary uppercase italic">{puzzle.hint}</span>
               </motion.div>
             )}
           </AnimatePresence>
        </div>
          ...
          <div className="space-y-6">
            <div className="flex flex-col gap-4">
              <input 
                ...
                className="w-full bg-zinc-950 border-2 border-zinc-800 rounded-2xl p-4 text-center text-xl font-bold text-white placeholder:text-zinc-700 focus:border-primary outline-none transition-all"
                value={input}
              ...
              <div className="flex items-center gap-3">
                <button 
                  onClick={handleGuess}
                  disabled={!input}
                  className="flex-1 bg-white text-zinc-950 py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-primary transition-all disabled:opacity-50"
                >
                  Confirmar Tentativa
                </button>
                ...
                {!showHint && (
                  <button 
                    ...
                    className="px-6 py-4 bg-zinc-800 text-zinc-400 rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-zinc-700 hover:text-white transition-all disabled:opacity-20 flex items-center gap-2"
            ...
            <div className="flex items-center justify-center gap-6">
               <div className="flex items-center gap-2">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-widest text-zinc-600">Tentativas: {attempts}/{puzzle.maxAttempts}</span>
               </div>
               <div className="w-px h-3 bg-zinc-800" />
               <div className="flex items-center gap-2">
                 <Keyboard className="w-3 h-3 text-zinc-600" />
                 <span className="text-[10px] font-black uppercase tracking-widest text-zinc-600">Pressione ENTER</span>
               </div>
            </div>
          </div>
        )}
        ...
        {isSolved && (
          ...
          >
            ...
            <span className="font-black uppercase tracking-[0.3em] italic">Enigma Resolvido</span>
          </motion.div>
        )}
        ...
        {isFailed && (
          ...
          >
            ...
            <span className="font-black uppercase tracking-[0.3em] italic">Portal Bloqueado</span>
          </motion.div>
        )}
      </div>
    </div>

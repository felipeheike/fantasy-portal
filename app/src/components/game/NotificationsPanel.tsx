      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 h-full w-full max-w-md bg-zinc-900 border-l border-zinc-800 shadow-2xl z-[130] flex flex-col"
          >
            {/* Header */}
            <div className="p-8 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/30 backdrop-blur-md">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-black uppercase tracking-tighter text-zinc-100">Sinalizador</h2>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
              {notificationHistory.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center opacity-30 py-20">
                  ...
                  <p className="text-zinc-500 font-serif italic text-lg">Silêncio no portal...</p>
                  ...
                  {notificationHistory.map((notif) => (
                    <motion.div
                      ...
                      className={`p-4 rounded-2xl border transition-all relative overflow-hidden ${
                        notif.read ? 'bg-zinc-800/20 border-zinc-800/50' : 'bg-zinc-800/40 border-zinc-700 ring-1 ring-primary/10'
                      }`}
                    >
                      {!notif.read && (
                      ...
                      <div className="flex gap-4">
                        <div className="p-2 bg-zinc-900 rounded-xl h-fit border border-zinc-800">
                          {getIcon(notif.type)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1">
                            <h3 className={`text-xs font-bold truncate ${notif.read ? 'text-zinc-400' : 'text-zinc-100'}`}>
                              {notif.title}
                            </h3>
                          ...
                          {notif.description && (
                            <p className="text-[10px] text-zinc-500 leading-relaxed font-serif italic line-clamp-2">
                              {notif.description}
                            </p>
                          )}
                        </div>
